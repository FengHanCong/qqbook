<template>
  <div class="bookDetails">
    <header id="header" class="mui-bar mui-bar-nav">
      ...
      <h1 class="mui-title">{{bookData1.classBook}}</h1>
    </header>
    <div ref="container" class="demo-loadmore-content">
      <mu-load-more @cancelable="cancelable" @refresh="refresh" :refreshing="refreshing" :loading="loading" @load="load">
        <mu-list>
          <div class="height"></div>
          <!--书内容-->
          <div class="content" v-for="btn in bookData" :key="btn.cover" @click="goDetails(btn)">
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.bookData = this.$route.params.id.fantasyData
      this.bookData1 = this.$route.params.id
//      console.log(this.$route.params.id.fantasyData)
    },
    methods: {
      go() {
        Toast.loading({
